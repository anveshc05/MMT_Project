...
'''
    ...
    signal processing
'''
def extract_feature(path, emotion):
    id = 1  # Song ID
    ...
    feature_set.to_csv('Test_Features/' + emotion + '_features.csv')
    feature_set.to_json('Test_Features/' + emotion + '_features.json')
    
# Extracting Feature Function Call
lis = ["Sadness", "Fear", "Anger"]
for folder in lis:
	extract_feature('../Final_data/Song_data/' + folder + '/', folder)